...
_FLAG = opt(regex(r'\s*\(\w*?\s*fragmented\)', value=Ignore))
...
_PROPS = opt(nt('PROPS', value=_make_props), default=None)
_EDGES = nt('EDGES')
...
_eds_parser = Peg(
    grammar=dict(
        ...
        CARG=bounded(lit('('), choice(DQString, lit('*TOP*')), lit(')')),
        PROPS=choice(bounded(lit('{'),
                             seq(_TYPE, _SPACES, _AVLIST),
                             lit('}')),
                     bounded(lit('{'), seq(_TYPE), lit('}'))),
