...
from .const_tree import ConstTree, Lexicon  # noqa
from .hyper_graph import HyperGraph  # noqa
...
def lowest_common_ancestor(tree_nodes, nodes):
    for node in tree_nodes:
        ...
        if all(i in all_children for i in nodes):
            return node
    raise Exception('No common ancestor for {}'.format(nodes))


class NodeDistributor:
    def __init__(self, hyper_graph, cfg_root, graph_type, logger=LOGGER):
        """
        @type hyper_graph: HyperGraph
        @type cfg_root: ConstTree
        """
    ...
    def _log_mapping(self, tree_node, pred_edge, reason=''):
        self.logger.debug('Map %s to %s (%s)',
                          pred_edge.label,
    ...
    def _group_tree_nodes_by_span(self):
        ...
        assert not redundant_pred_edges, 'Redundant nodes in graph: {}'.format(redundant_pred_edges)

        return span_to_pred_edges, span_to_tree_nodes, structural_edges

    def _solve_first_try(self, span_to_tree_nodes, span_to_pred_edges):
        many_to_many = []
    ...
    def _solve_leaf(self, leaf_nodes, pred_edges, structural_edges):
        ...
        for edge in pred_edges:
            if edge.label in SPECIAL_LEXICAL_EDGE:
                ...
                self.attachment_waiting_list[edge.nodes[0], direction, which_label] = edge
            else:
                raise Exception('Don\'t know how to solve edge {}'.format(edge))
        ...
        if rest_lexical_edges:
            raise Exception('Don\'t know how to solve these edges {}'.format(
                list(rest_lexical_edges.keys())
            ))
    ...
    def _solve_internal(self, pred_edges_list, tree_nodes, structural_edges):
        for pred_edges in pred_edges_list:
            ...
            if top_waiting:
                raise Exception('Don\'t know how to solve these edges {}'.format(list(top_waiting)))

        self._solve_attachment(structural_edges)

        if self.attachment_waiting_list:
            values = list(self.attachment_waiting_list.values())
            raise Exception('Don\'t know how to solve these edges {}'.format(values))
